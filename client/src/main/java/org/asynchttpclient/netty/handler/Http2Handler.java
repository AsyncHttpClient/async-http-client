...
public final class Http2Handler extends AsyncHttpClientHandler {
    ...
    private void handleHttp2HeadersFrame(Http2HeadersFrame headersFrame, Channel channel,
        ...
        HttpResponse syntheticResponse = new DefaultHttpResponse(HttpVersion.HTTP_1_1, nettyStatus, responseHeaders);

        future.setKeepAlive(true); // HTTP/2 connections are always multiplexed/kept-alive
    ...
    void finishUpdate(NettyResponseFuture<?> future, Channel streamChannel, boolean close) {
        ...
        if (!close && parentChannel != null && parentChannel.isActive()) {
