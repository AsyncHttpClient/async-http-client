...
public class Uri {
  ...
  /**
   * Same as {@link #toUrl()} but without the deprecated userinfo subcomponent. RFC 9110 section 4.2.4: a
   * sender MUST NOT generate the userinfo subcomponent (and its "@" delimiter) when an origin-form or
   * absolute-form URI reference is generated for a request target, so this is what belongs on the wire when
   * an absolute-form target is required. {@link #toUrl()} keeps the userinfo for the caller-visible URL.
   *
