        ...
        if (statusCode == UNAUTHORIZED.code() && realm != null && !future.getAndSetAuth(true)) {
            ...
            if (!wwwAuthHeaders.isEmpty()) {
                ...
                if (!wwwAuthHeaders.contains("Kerberos") && ntlmAuthenticate != null) {
                    // NTLM
                	Method m = c.getDeclaredMethod("ntlmChallenge", new Class[]{String.class, Request.class, ProxyServer.class, FluentCaseInsensitiveStringsMap.class, Realm.class,
                				NettyResponseFuture.class, boolean.class});
                	m.setAccessible(true);
        			newRealm = (Realm)m.invoke(obj, ntlmAuthenticate, request, proxyServer, request.getHeaders(), realm, future, false);
                	...
                	Method m = c.getDeclaredMethod("kerberosChallenge", new Class[]{Channel.class, List.class, Request.class, ProxyServer.class, FluentCaseInsensitiveStringsMap.class, Realm.class,
                            NettyResponseFuture.class, boolean.class});
        			m.setAccessible(true);
        			newRealm = (Realm)m.invoke(channel, wwwAuthHeaders, request, proxyServer, request.getHeaders(), realm, future, false);
                    // SPNEGO KERBEROS
                    if (newRealm == null)
                        return true;
                    else
